  ...
  if (req.method === 'POST' && pathname === '/admin/role') {
    ...
    req.on('end', () => {
      try {
        ...
        users[key].adminRole = role; // null = no special role
        if (role === 'founder') users[key].runeBadge = true; // founder always has badge
        saveUsers().catch(console.error);
        // Broadcast badge update to all clients so badge appears live
        const displayName = users[key].name;
        broadcast({ type: 'badge_update', name: displayName, hasBadge: role !== null, adminRole: role });
        res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    ...
    if (data.type === 'register') {
      ...
      if (btc) {
        checkRuneBadge(btc).then(({ badge }) => {
          if (users[nameKey] && users[nameKey].adminRole !== 'founder') {
            users[nameKey].runeBadge = badge;
            saveUsers().catch(console.error);
          }
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name, hasBadge: badge, adminRole: users[nameKey]?.adminRole || null }));
        });
      }
    }
    ...
    if (data.type === 'update_user') {
      ...
      if (newBtc) {
        checkRuneBadge(newBtc).then(({ badge }) => {
          if (users[newKey] && users[newKey].adminRole !== 'founder') {
            users[newKey].runeBadge = badge;
            saveUsers().catch(console.error);
          }
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name: newName, hasBadge: badge, adminRole: users[newKey]?.adminRole || null }));
        });
      }
    }
