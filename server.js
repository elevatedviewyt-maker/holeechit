...
const ORDISCAN_KEY = process.env.ORDISCAN_KEY || ''; // optional, raises rate limits

function has369(val) {
  // true if the integer part of val contains "369" as substring
...
async function checkRuneBadge(btcAddress) {
  ...
  try {
    ...
    const magic = txs.some(tx => {
      const candidates = [
        tx.amount, tx.value, tx.runeAmount,
        tx.receiveAmount, tx.sendAmount, tx.transferAmount
      ].filter(v => v != null);
      return candidates.some(v => has369(v));
    });

    if (!magic) return { badge: false, reason: 'no tx with 369 in amount' };

    console.log(`✓ Badge earned: ${btcAddress.slice(0,10)}… holds ${balance}, has 369-tx`);
    return { badge: true, reason: 'ok' };

  } catch (e) {
    console.error('checkRuneBadge error:', e.message);
    return { badge: false, reason: 'api_error' }; // fail open — no badge, user still joins
  }
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (msg) => {
    ...
    if (data.type === 'register') {
      ...
      if (btc) {
        checkRuneBadge(btc).then(({ badge }) => {
          ...
          saveUsers().catch(console.error);
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name, hasBadge: badge }));
        });
      }
    }
    ...
    if (data.type === 'update_user') {
      ...
      if (newBtc) {
        checkRuneBadge(newBtc).then(({ badge }) => {
          ...
          saveUsers().catch(console.error);
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name: newName, hasBadge: badge }));
        });
      }
    }
    ...
    if (data.type === 'chat') {
      ...
      const runeBadge = users[name.toLowerCase()]?.runeBadge || false;
      const chatMsg = { name, text, ts: Date.now(), runeBadge };
