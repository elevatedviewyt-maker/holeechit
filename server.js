...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (msg) => {
    ...
    if (data.type === 'register') {
      ...
      // Accept both 'eth' and 'btc' field names (eth takes priority)
      const btc  = (data.eth || data.btc || '').trim();
      if (!name) return ws.send(JSON.stringify({ type: 'register_err', msg: 'Name required' }));

      // Validate ETH address format if provided
      if (btc && !/^0x[0-9a-fA-F]{40}$/.test(btc)) {
        return ws.send(JSON.stringify({ type: 'register_err', field: 'btc', msg: 'Invalid ETH wallet address (must start with 0x and be 42 characters)' }));
      }
    ...
    if (data.type === 'update_user') {
      ...
      // Accept both 'newEth' and 'newBtc' field names
      const newBtc  = (data.newEth || data.newBtc || '').trim();
      if (!oldName || !newName)
        return ws.send(JSON.stringify({ type: 'update_err', msg: 'Name required' }));

      // Validate ETH address format if provided
      if (newBtc && !/^0x[0-9a-fA-F]{40}$/.test(newBtc)) {
        return ws.send(JSON.stringify({ type: 'update_err', field: 'btc', msg: 'Invalid ETH wallet address (must start with 0x and be 42 characters)' }));
      }
