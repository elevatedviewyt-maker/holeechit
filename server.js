...
// ── Rune badge config ─────────────────────────────────────
// Set RUNE_NAME env var on Render when you have your rune (e.g. HOLEE•CHIT)
const RUNE_NAME    = process.env.RUNE_NAME    || 'PLACEHOLDER•RUNE';
const ORDISCAN_KEY = process.env.ORDISCAN_KEY || '';

function has369(val) {
  return String(Math.floor(parseFloat(val) || 0)).includes('369');
}

async function ordiscanFetch(path) {
  const headers = { 'Accept': 'application/json' };
  if (ORDISCAN_KEY) headers['Authorization'] = `Bearer ${ORDISCAN_KEY}`;
  const resp = await fetch(`https://api.ordiscan.com/v1${path}`, {
    headers, signal: AbortSignal.timeout(8000)
  });
  if (!resp.ok) throw new Error(`Ordiscan ${resp.status}: ${path}`);
  return resp.json();
}

async function checkRuneBadge(btcAddress) {
  if (!btcAddress) return { badge: false, reason: 'no address' };
  const addrEnc = encodeURIComponent(btcAddress);
  const nameEnc = encodeURIComponent(RUNE_NAME);
  try {
    // Condition 1: holds >= 369 of the rune right now
    const balData = await ordiscanFetch(`/address/${addrEnc}/runes`);
    const runes   = balData?.data || balData?.result || [];
    const myRune  = runes.find(r =>
      r.name?.toUpperCase()       === RUNE_NAME.toUpperCase() ||
      r.spacedName?.toUpperCase() === RUNE_NAME.toUpperCase()
    );
    if (!myRune) return { badge: false, reason: 'does not hold rune' };

    const balance = parseFloat(myRune.balance ?? myRune.amount ?? 0);
    if (balance < 369) return { badge: false, reason: `holds ${balance} < 369` };

    // Condition 2: at least one tx where amount contains "369"
    const actData = await ordiscanFetch(`/address/${addrEnc}/rune-activity?name=${nameEnc}`);
    const txs = actData?.data || actData?.result || [];
    const magic = txs.some(tx => {
      const vals = [tx.amount, tx.value, tx.runeAmount, tx.receiveAmount, tx.sendAmount, tx.transferAmount].filter(v => v != null);
      return vals.some(v => has369(v));
    });
    if (!magic) return { badge: false, reason: 'no tx with 369 in amount' };

    console.log(`✓ Badge: ${btcAddress.slice(0,10)}… holds ${balance}, has 369-tx`);
    return { badge: true, reason: 'ok' };
  } catch (e) {
    console.error('checkRuneBadge error:', e.message);
    return { badge: false, reason: 'api_error' };
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  // POST /admin/role?pass=xxx  body:{name, role:'founder'|'holder'|null}
  if (req.method === 'POST' && pathname === '/admin/role') {
    if (!checkAdmin(req, res)) return;
    let body = '';
    req.on('data', d => body += d);
    req.on('end', () => {
      try {
        ...
        const key = (name || '').toLowerCase();
        if (!key) { res.writeHead(400); return res.end(JSON.stringify({ error: 'name required' })); }
        if (!users[key]) { res.writeHead(404); return res.end(JSON.stringify({ error: 'user not found' })); }
        // role can be 'founder', 'holder', or null to remove
        const validRoles = ['founder', 'holder', null];
        if (!validRoles.includes(role)) { res.writeHead(400); return res.end(JSON.stringify({ error: 'invalid role' })); }
        users[key].adminRole = role;
        if (role === 'founder') {
          users[key].runeBadge = true;
          users[key].adminOverride = false; // founder = re-enable auto check too
        } else if (role === 'holder') {
          users[key].runeBadge = true;
          users[key].adminOverride = false;
        } else if (role === null) {
          users[key].runeBadge = false;
          users[key].adminOverride = true; // block auto wallet check from re-enabling
        }
        saveUsers().catch(console.error);
        const displayName = users[key].name;
        const hasBadge = role !== null;
        broadcast({ type: 'badge_update', name: displayName, hasBadge, adminRole: role });
        res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ ok: true, name: displayName, role }));
      } catch(e) { res.writeHead(400); res.end(JSON.stringify({ error: 'Invalid JSON' })); }
    });
    return;
  }
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (msg) => {
    ...
    if (data.type === 'register') {
      ...
      // Async rune badge check — doesn't block registration
      if (btc) {
        checkRuneBadge(btc).then(({ badge }) => {
          // Don't override if admin manually removed the role
          if (users[nameKey] && !users[nameKey].adminOverride) {
            users[nameKey].runeBadge = badge;
            saveUsers().catch(console.error);
          }
          const finalBadge = users[nameKey]?.runeBadge || false;
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name, hasBadge: finalBadge, adminRole: users[nameKey]?.adminRole || null }));
        });
      }
    }
    ...
    if (data.type === 'update_user') {
      ...
      if (newBtc) {
        checkRuneBadge(newBtc).then(({ badge }) => {
          if (users[newKey] && !users[newKey].adminOverride) {
            users[newKey].runeBadge = badge;
            saveUsers().catch(console.error);
          }
          const finalBadge = users[newKey]?.runeBadge || false;
          if (ws.readyState === 1)
            ws.send(JSON.stringify({ type: 'badge_update', name: newName, hasBadge: finalBadge, adminRole: users[newKey]?.adminRole || null }));
        });
      }
    }
    ...
    if (data.type === 'chat') {
      ...
      const senderUser = users[name.toLowerCase()] || {};
      const chatMsg = { name, text, ts: Date.now(), runeBadge: senderUser.runeBadge || false, adminRole: senderUser.adminRole || null };
